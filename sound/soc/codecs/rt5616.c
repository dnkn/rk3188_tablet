...
static struct rt5616_init_reg init_list[] = {
	...
	//{RT5616_ADC_DIG_VOL	, 0x4f4f}, //mute adc for pop noise
	{RT5616_ADC_DIG_VOL	, 0x5757},
	
	/*MIC*/
	{RT5616_STO1_ADC_MIXER	, 0x3802},
	{RT5616_IN1_IN2		, 0x0100} //20db
	//{RT5616_IN1_IN2		, 0x8800} //20db
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	//printk("rt5616_adc_event   event =%d \n",event);
	switch (event) {
...
extern  void codec_set_spk(int status);

//static int fg_spk = 1;
extern int fg_hdmi_open;
...
//printk("enter %s\n",__func__);
 ...
 void codec_set_spk(int status)
{


	gpio_request(RK30_PIN2_PD7, "null");

			//printk("------------sk--------------codec_set_spk  =%d   \n",status);
...
//printk("enter %s\n",__func__);
	struct snd_soc_codec *codec = w->codec;
//printk("rt5616_bst1_event   event =%d \n",event);
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_update_bits(codec, RT5616_PWR_ANLG2,
...
//printk("enter %s\n",__func__);
	struct snd_soc_codec *codec = w->codec;
//printk("rt5616_bst2_event   event =%d \n",event);
		    ...
		    const struct i2c_device_id *id)
{
	...
	if (NULL == rt5616)
		...
		//printk("wq ---------- rk5616_codec_io_init   \n ");

	//rt5616_spk_ctrl_fun(1);
	//codec_set_spk(1);
